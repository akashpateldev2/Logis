...
function site_options_menu() {
    ...
    foreach($ters as $site_option) 
    {
        ...
        elseif($site_option['menu_name']!=$ters[0]['menu_name'] AND $site_option['is_procted']=='yes' AND (get_option('developermode')=='enable' OR isset($_GET['developermode']) && $_GET['developermode']=='enable'))
        {
            add_submenu_page( $ters[0]['page_slug'], $site_option['menu_name'], $site_option['menu_name'], 'administrator', $site_option['page_slug'], $site_option['page_slug'] );
        }        
    }
...
";
    ...
    elseif($site_option['is_procted']=='yes' AND (get_option('developermode')=='enable' OR isset($_GET['developermode']) && $_GET['developermode']=='enable'))
    {
        include $site_option['php_page_name'];
    }
